...
if (($_POST['action'] ?? '') === 'create') {
	...
	if (!$errors) {
		try {
			execute_rest_insert('editora', [
				'ed_nome' => $ed_nome,
				'ed_morada' => $ed_morada,
				'ed_email' => $ed_email,
				'ed_codpostal' => $ed_codpostal,
				'ed_tlm' => $ed_tlm
			]);
...
if (($_POST['action'] ?? '') === 'delete') {
	...
	if ($ed_cod <= 0) {
		...
		try {
			execute_rest_delete('editora', ['ed_cod' => $ed_cod]);
...
// Operação delete_all removida - agora cada editora tem botão individual

// Dados para listagens usando API REST
$editoras = [];
$codigosPostais = [];
try {
	$editoras = execute_rest_sql('editora', ['ed_cod', 'ed_nome', 'ed_morada', 'ed_email', 'ed_codpostal', 'ed_tlm'], [], 'ed_nome');
	$codigosPostais = execute_rest_sql('codigo_postal', ['cod_postal', 'cod_localidade'], [], 'cod_postal');
} catch (Throwable $e) {
