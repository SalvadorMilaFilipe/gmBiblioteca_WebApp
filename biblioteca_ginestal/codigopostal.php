...
if (($_POST['action'] ?? '') === 'create') {
	...
	if (!$errors) {
		try {
			execute_rest_insert('codigo_postal', [
...
if (($_POST['action'] ?? '') === 'update') {
	...
	if (!$errors) {
		try {
			execute_rest_update('codigo_postal', [
				'cod_localidade' => $cod_localidade
			], ['cod_postal' => $cod_postal_pk]);
...
if (($_POST['action'] ?? '') === 'delete') {
	...
	if ($cod_postal_pk === '') {
		...
		try {
			execute_rest_delete('codigo_postal', ['cod_postal' => $cod_postal_pk]);
...
// List usando API REST
$lista = [];
try {
	$lista = execute_rest_sql('codigo_postal', ['cod_postal', 'cod_localidade'], [], 'cod_postal');
} catch (Throwable $e) {
	$errors[] = 'Erro ao carregar códigos postais: ' . $e->getMessage();
}
?>
<!doctype html>
<html lang="pt-br">
