...
if (($_POST['action'] ?? '') === 'create') {
	...
	if (!$errors) {
		try {
			$stmt = $pdo->prepare('INSERT INTO genero (ge_genero) VALUES (?)');
			$stmt->execute([$ge_genero]);
...
if (($_POST['action'] ?? '') === 'update') {
	...
	if (!$errors) {
		try {
			$stmt = $pdo->prepare('UPDATE genero SET ge_genero = ? WHERE ge_genero = ?');
			$stmt->execute([$ge_genero_new, $ge_genero_pk]);
...
if (($_POST['action'] ?? '') === 'delete') {
	...
	if ($ge_genero_pk === '') {
		...
		try {
			$stmt = $pdo->prepare('DELETE FROM genero WHERE ge_genero = ?');
			$stmt->execute([$ge_genero_pk]);
...
$lista = [];
try {
	$lista = $pdo->query('SELECT ge_genero FROM genero ORDER BY ge_genero')->fetchAll();
} catch (Throwable $e) {
