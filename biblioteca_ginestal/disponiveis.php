...
if (($_POST['action'] ?? '') === 'add_book') {
	...
	if (!$errors) {
		try {
			...
			$livro_cod = create_book($titulo, $autor_string, $isbn, $editora, $idioma, $edicao, $ano, $genero);
...
try {
	...
	$errors[] = 'Erro ao carregar livros: ' . $e->getMessage();
	$lista = [];
}
?>
<!doctype html>
...
<body>
				...
				<div class="card shadow-sm">
					<div class="card-header">
						<div class="d-flex justify-content-between align-items-center mb-3">
							<strong>Catálogo de Livros</strong>
							<span class="badge bg-primary"><?= count($lista) ?> livro(s) encontrado(s)</span>
						</div>
					...
					<div class="table-responsive">
						<table class="table table-striped mb-0">
							...
							<tbody>
                                <?php if (!$lista): ?>
                                    <tr><td colspan="10" class="text-center text-muted">Sem resultados.</td></tr>
								<?php else: foreach ($lista as $livro): ?>
									<tr>
										<td><span class="badge bg-secondary"><?= (int)$livro['li_cod'] ?></span></td>
										<td><strong><?= htmlspecialchars((string)$livro['li_titulo'], ENT_QUOTES, 'UTF-8') ?></strong></td>
										<td><?= htmlspecialchars((string)($livro['autor_nome'] ?? $livro['li_autor'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
										<td><code><?= htmlspecialchars((string)$livro['li_isbn'], ENT_QUOTES, 'UTF-8') ?></code></td>
										<td><?= htmlspecialchars((string)($livro['editora_nome'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
										<td><?= htmlspecialchars((string)$livro['li_idioma'], ENT_QUOTES, 'UTF-8') ?></td>
										<td><?= (int)$livro['li_edicao'] ?>ª</td>
										<td><?= (int)$livro['li_ano'] ?></td>
										<td><span class="badge bg-info"><?= htmlspecialchars((string)$livro['li_genero'], ENT_QUOTES, 'UTF-8') ?></span></td>
                                        <td>
											<button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#exemplaresModal" 
													onclick="mostrarExemplares(<?= (int)$livro['li_cod'] ?>, '<?= htmlspecialchars((string)$livro['li_titulo'], ENT_QUOTES, 'UTF-8') ?>')">
												Ver Exemplares
											</button>
										</td>
									</tr>
							...
							<div class="mb-3">
								...
								<select class="form-select" id="editora" name="editora" required>
									...
									<?php foreach ($editoras as $ed): ?>
										<option value="<?= (int)$ed['ed_cod'] ?>"><?= htmlspecialchars($ed['ed_nome'], ENT_QUOTES, 'UTF-8') ?></option>
									<?php endforeach; ?>
								</select>
	...
	(function() {
		...
		function performSearch() {
			...
			searchTimeout = setTimeout(() => {
				...
				.catch(error => {
					...
					if (tbody) {
						tbody.innerHTML = '<tr><td colspan="10" class="text-center text-danger py-3">Erro ao pesquisar livros</td></tr>';
					}
					isLoading = false;
				});
		...
		function updateTable(data) {
			...
			if (data.length === 0) {
				tbody.innerHTML = '<tr><td colspan="10" class="text-center text-muted">Sem resultados.</td></tr>';
				return;
			}
			...
			data.forEach(livro => {
				...
				row.innerHTML = `
					<td><span class="badge bg-secondary">${livro.li_cod}</span></td>
					<td><strong>${escapeHtml(livro.li_titulo)}</strong></td>
					<td>${escapeHtml(livro.autor_nome || livro.li_autor || '')}</td>
					<td><code>${escapeHtml(livro.li_isbn || '')}</code></td>
					<td>${escapeHtml(livro.editora_nome || '')}</td>
					<td>${escapeHtml(livro.li_idioma || '')}</td>
					<td>${livro.li_edicao}ª</td>
					<td>${livro.li_ano}</td>
					<td><span class="badge bg-info">${escapeHtml(livro.li_genero)}</span></td>
					<td>
						<button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#exemplaresModal" 
